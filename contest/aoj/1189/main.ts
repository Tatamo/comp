...
const [m, n] = input[0].split(" ").map((x: string): number => +x);

const sieve = (n: number): Array<boolean> => {
...
const stage = (p: Array<boolean>, n: number) => {
...
const mkgraph = (stage: Array<Array<number | null>>, n: number): Array<Array<Set<Point>>> => {
	const include = new Array(n + 1).fill(new Set());
	const size = Math.ceil(Math.sqrt(n));
	const map: Array<Array<Set<Point>>> = new Array(size);
	for (let i = 0; i < size; i++) {
	...
	for (let y = 0; y < size; y++) {
		for (let x = 0; x < size; x++) {
			...
			for (let i = -1; i <= 1; i++) {
				const ny = y + 1;
				const nx = x + i;
				if (ny >= size || nx < 0 || nx >= size) continue;
				// 包含解消しつつ追加
				outer:
					for (const p1 of map[y][x]) {
						for (const p2 of map[ny][nx]) {
							if (p2.children.has(p1)) continue outer;
							else if (p1.children.has(p2)) map[ny][nx].delete(p2);
						}
						map[ny][nx].add(p1);
					}
			}
...
const solve = (stage: Array<Array<number | null>>, n: number, start: number) => {
