...
const mi = (x: number) => new ModInt(x, MOD);
...
memo[1] = [];
memo[1][0] = m;
for (let i = 1; i <= k; i++) memo[1][i] = 0;

const rsw = new Array<Array<number>>();
rsw[1] = new Array<number>(k + 1).fill(m);

const foo = new Set();
let cnt = 0;
function _solve(n: number, k: number): number {
...
function solve(n: number, k: number): number {
	...
	if (memo[n] === undefined) memo[n] = [];
	if (memo[n][k] !== undefined) return memo[n][k];
	if (n === 1) return memo[1][k];
	let result = 0;
	let left_n = n % 2 === 0 ? n / 2 : 1 << n.toString(2).length - 1;
	let right_n = n % 2 === 0 ? n / 2 : n - left_n;

	for (let i = 0; i <= k; i++) {
		const l = _solve(left_n, i);
		const r = _solve(right_n, k - i);
		result = mi(l).mul(r).mul(m - 1).div(m).add(result).value;
	}
	for (let i = 0; i < k; i++) {
		const l = _solve(left_n, i);
		const r = _solve(right_n, k - i - 1);
		result = mi(l).mul(r).div(m).add(result).value;
	}
	memo[n][k] = result;
	return result;
}
