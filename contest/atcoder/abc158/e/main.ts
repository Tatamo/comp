...
function main(input: Array<string>) {
  ...
  if (p === 2 || p === 5) {
    ...
    let result = 0;
    const mods = new Array(n + 1);
    mods[n] = new ModInt(0, p);
    for (let i = n - 1; i >= 0; i--) {
      mods[i] = (new ModInt(10, p).pow(n - i - 1).mul(+s[i]).add(mods[i + 1]));
    }
    const rsw_mods = new Array(p).fill(0);
    for (let i = n - 1; i >= 0; i--) {
      result += rsw_mods[mods[i].value];
      rsw_mods[mods[i].value]++;
      if (mods[i].value === 0) result++;
    }
    return result;
  }
}
