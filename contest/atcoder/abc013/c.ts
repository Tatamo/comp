...
const [n, h] = input[0].split(" ").map((x: string): number => +x);
const [a, b, c, d, e] = input[1].split(" ").map((x: string): number => +x);

const check = (x: number, y: number, z: number): { health: number, cost: number } => {
...
for (let x = 0; x <= n; x++) {
	...
	for (let i = Math.max(0, left - 3); i <= Math.min(n, right + 3); i++) {
		...
		if (health > 0 && cost < result) result = cost;
	}
}
console.log(result);
