...
for(let d=0; d<4; d++){
	for(let i=range[d][0] as number;;i+=range[d][2] as number){
		for(let ii=range[d][3] as number;;ii+=range[d][5] as number){
			const c = (range[d][6]==="v")?a[i][ii]:a[ii][i];
			if(c!=="."){
				read[d] += c;
				break;
			}
			if(ii===range[d][4]) break;
		}
		if(i===range[d][1]) break;
	}
