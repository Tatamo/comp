...
for (let i = 0; i < n; i++) {
	const [x, y] = input[i + 1].split(" ").map((x: string): number => +x);
	points.push({x, y, index: i, id: i});
}

const rotate = (p: Point, d: number): { x: number, y: number } => {
	return {x: p.x * Math.cos(d) - p.y * Math.sin(d), y: p.x * Math.sin(d) + p.y * Math.cos(d)};
};
console.time("test");

const dist: Array<Array<number>> = new Array(n).fill([]).map(() => new Array(n));
for (let i = 0; i < n; i++) {
...
const map: Array<Array<number>> = new Array(n).fill([]).map(() => []);
const sorted: Array<Array<Point>> = [];
...
const sp = (i1: number, i2: number, i3: number) => {
	const a = dist[i1][i2];
	const b = dist[i2][i3];
	const c = dist[i3][i1];
