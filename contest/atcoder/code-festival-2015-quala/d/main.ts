...
function check(n:number, x:Array<number>, s:number): boolean{
	var left = 0;
	var right = 0;
	//console.log("s=",s);
	for(var i=0; i<x.length; i++){
		//console.log(i,"th:");
		if(i==0) left = x[i]-1;
		else left = Math.max(x[i]-x[i-1]-1-right,0);
...
function Main(input) {
    ...
    for (var i = 0; i < m; i++) {
		x[i] = +input[i];
		if(i==0) dmax = x[i]-1;
		if(i!=0 && x[i]-x[i-1] > dmax) dmax = x[i]-x[i-1];
		if(i==m-1 && n-x[i]>dmax) dmax = n-x[i];
    }
	var result = 0;
