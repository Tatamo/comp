...
const C = 0;
const J = 1;
const getOpposite = (p: number): number => {
	if (p === C) return J;
	if (p === J) return C;
	throw new Error();
};
type Time = { person: number, l: number, r: number };

const solve = (problem_number: number, ac: number, aj: number, times: Array<Time>) => {
	let remain = [720, 720];
	let fixed = [0, 0];
	let fix_section: Array<Array<number>> = [[], []];
	let ftime = 0;
	let exchange = 0;
	...
	if (fixed[C] > remain[C] || fixed[J] > remain[J]) {
		if (fixed[C] > remain[C] && fixed[J] > remain[J]) console.log("おかしい！！！！！！！");
...
for (let i = 0; i < t; i++) {
	const [ac, aj] = input[line++].split(" ").map((x: string): number => +x);
	const times: Array<Time> = [];
	for (let ii = 0; ii < ac; ii++) {
		const [l, r] = input[line++].split(" ").map((x: string): number => +x);
